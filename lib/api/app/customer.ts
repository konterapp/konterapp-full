...
export async function getCustomers(
  ...
  const response = await fetch(`/api/app/pos/customers?${params}`);
...
export async function getCustomer(uuid: string): Promise<{ status: string; data: Customer }> {
  const response = await fetch(`/api/app/pos/customers/${uuid}`);
  return response.json();
}
...
export async function createCustomer(data: {
  ...
  const response = await fetch('/api/app/pos/customers', {
    method: 'POST',
