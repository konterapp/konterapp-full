...
export async function getUsers(
  ...
  return apiRequest<PaginatedData<User>>(`/api/app/users?${params.toString()}`);
}

export async function getUser(uuid: string): Promise<ApiResponse<User>> {
  return apiRequest<User>(`/api/app/users/${uuid}`);
}
...
export async function createUser(data: UserCreateData): Promise<ApiResponse<User>> {
  ...
  return apiRequest<User>('/api/app/users', {
    method: 'POST',
    data: formData,
  });
}
...
export async function deleteUser(uuid: string): Promise<ApiResponse<void>> {
  return apiRequest<void>(`/api/app/users/${uuid}`, {
    method: 'DELETE',
  });
}
...
export async function getRoles(): Promise<ApiResponse<Role[]>> {
  return apiRequest<Role[]>('/api/app/users/roles');
}

export async function toggleUserActive(uuid: string): Promise<ApiResponse<User>> {
  return apiRequest<User>(`/api/app/users/${uuid}/toggle-active`, {
    method: 'PATCH',
  });
}

export async function impersonateUser(userId: number): Promise<ApiResponse<{ user: User; impersonating: boolean }>> {
  return apiRequest<{ user: User; impersonating: boolean }>(`/api/app/impersonate/users/${userId}`, {
    method: 'POST',
  });
}
