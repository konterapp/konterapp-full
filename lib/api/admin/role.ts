...
export async function getRoles(
  ...
  return apiRequest<PaginatedData<Role>>(`/api/admin/roles?${params.toString()}`);
}

export async function getRole(id: number): Promise<ApiResponse<Role>> {
  return apiRequest<Role>(`/api/admin/roles/${id}`);
}
...
export async function deleteRole(id: number): Promise<ApiResponse<void>> {
  return apiRequest<void>(`/api/admin/roles/${id}`, {
    method: 'DELETE',
  });
}

export async function getPermissions(): Promise<ApiResponse<Permission[]>> {
  return apiRequest<Permission[]>('/api/admin/roles/permissions');
}
