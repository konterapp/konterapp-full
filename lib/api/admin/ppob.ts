...
export async function getPpobProductsByCategory(category: string, brand?: string): Promise<ApiResponse<PpobProductLocal[]>> {
  ...
  return apiRequest<PpobProductLocal[]>(`/api/admin/pos/ppob-products/by-category?${params.toString()}`);
}

export async function getPpobBrandsByCategory(category: string): Promise<ApiResponse<string[]>> {
  return apiRequest<string[]>(`/api/admin/pos/ppob-products/brands-by-category?category=${encodeURIComponent(category)}`);
}
...
export async function getPpobProductsList(params?: {
  ...
  return apiRequest<PpobProductLocal[]>(`/api/admin/pos/ppob-products?${searchParams.toString()}`);
}
...
export async function createPpobProduct(data: {
  ...
  return apiRequest<PpobProductLocal>('/api/admin/pos/ppob-products', {
    method: 'POST',
    data,
  });
}
...
export async function updatePpobProduct(uuid: string, data: Partial<{
  ...
  return apiRequest<PpobProductLocal>(`/api/admin/pos/ppob-products/${uuid}`, {
    method: 'PATCH',
    data,
  });
}
...
export async function deletePpobProduct(uuid: string): Promise<ApiResponse<never>> {
  return apiRequest<never>(`/api/admin/pos/ppob-products/${uuid}`, {
    method: 'DELETE',
  });
}

export async function syncPpobProducts(provider: string, category: string): Promise<ApiResponse<never>> {
  return apiRequest<never>('/api/admin/pos/ppob-products/sync', {
    method: 'POST',
...
export async function syncAllDigiflazz(): Promise<ApiResponse<Record<string, number>>> {
  return apiRequest<Record<string, number>>('/api/admin/pos/ppob-products/sync-all', {
    method: 'POST',
  });
}
...
export async function inquiryBill(
  ...
  const response = await apiRequest<never>('/api/admin/pos/ppob/inquiry', {
    method: 'POST',
    data: {
...
export async function purchasePrepaid(data: PurchasePrepaidData): Promise<ApiResponse<PpobTransaction>> {
  return apiRequest<PpobTransaction>('/api/admin/pos/ppob/purchase', {
    method: 'POST',
    data,
  });
}
...
export async function payPostpaid(data: PayPostpaidData): Promise<ApiResponse<PpobTransaction>> {
  return apiRequest<PpobTransaction>('/api/admin/pos/ppob/pay', {
    method: 'POST',
    data,
  });
}
...
export async function getPpobTransactions(
  ...
  return apiRequest<PpobTransaction[]>(`/api/admin/pos/ppob/transactions?${params.toString()}`);
}

export async function getPpobTransactionDetail(uuid: string): Promise<ApiResponse<PpobTransaction>> {
  return apiRequest<PpobTransaction>(`/api/admin/pos/ppob/transactions/${uuid}`);
}

export async function checkPpobTransactionStatus(uuid: string): Promise<ApiResponse<PpobTransaction>> {
  return apiRequest<PpobTransaction>(`/api/admin/pos/ppob/transactions/${uuid}/check-status`, {
    method: 'POST',
  });
}
...
export async function getProviderBalance(provider?: string): Promise<ProviderResponse> {
  ...
  const response = await apiRequest<never>(`/api/admin/pos/ppob/balance${params}`);
  return response as unknown as ProviderResponse;
}
