...
export default function PriceCheckPage() {
  ...
  const loadProductDetail = useCallback(
    async (uuid: string) => {
      ...
      try {
        const response = await fetch(`/api/app/pos/price-check/${uuid}`);
  ...
  const loadOptions = useCallback(async () => {
    try {
      const response = await fetch('/api/app/pos/price-check/options');
      const result = await response.json();
      if (result.status === 'success' && result.data) {
  ...
  const loadProducts = useCallback(
    async (query: string, branchUuid: string) => {
      ...
      try {
        ...
        const response = await fetch(`/api/app/pos/price-check?${params.toString()}`);
        const result = await response.json();
  ...
  const handleBarcodeScan = useCallback(
    async (code: string) => {
      ...
      try {
        ...
        const response = await fetch(`/api/app/pos/products/lookup-barcode?${params.toString()}`);
        const result = await response.json();
