...
import { ArrowLeft, Calendar, Building2, Zap, CreditCard, User, FileText } from 'lucide-react';
import { getPpobTransaction, PpobTransaction } from '@/lib/api/app/ppob-transaction';
...
export default function PpobTransactionDetailPage({ params }: { params: Promise<{ uuid: string }> }) {
  const resolvedParams = use(params);
  const [transaction, setTransaction] = useState<PpobTransaction | null>(null);
  ...
  const fetchTransaction = async (uuid: string) => {
    try {
      setIsLoading(true);
      setError('');
      const result = await getPpobTransaction(uuid);
      ...
      <div className="p-6">
        <div className="flex h-64 items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/app/pos/ppob-transactions" className="text-[#EBC170] hover:underline">
              Kembali ke daftar Server Pulsa/PPOB
            </Link>
          </div>
        </div>
      </div>
  ...
  const isRefund = transaction.cash_direction === 'in';

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link href="/app/pos/ppob-transactions" className="cursor-pointer rounded-lg p-2 transition-colors hover:bg-gray-100">
            <ArrowLeft className="h-5 w-5 text-gray-600" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-[#142D52]">Detail Transaksi Server Pulsa/PPOB</h1>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-sm text-gray-500">Akun Server PPOB</p>
                  <p className="text-sm font-medium text-gray-900">{transaction.account?.name || '-'}</p>
                  {transaction.account_reference && (
                    <p className="text-xs text-gray-500">No. Tujuan: {transaction.account_reference}</p>
                  )}
                </div>
              </div>
          ...
          <div className="sticky top-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                <span className="text-sm font-semibold text-gray-900">Modal</span>
                <span className="text-lg font-bold text-gray-900">{formatCurrency(transaction.base_amount)}</span>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">{isRefund ? 'Nominal Dikembalikan' : 'Harga Jual'}</span>
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-3">
                <span className="text-sm text-gray-600">Biaya Admin PPOB</span>
