...
export default function BankAgentTransactionsPage() {
  ...
  // Tampilan kartu untuk layar kecil -- tabel ini punya 10 kolom, kalau
  // dipaksa jadi tabel di HP harus digeser-geser ke samping dan sebagian
  // besar kolomnya tidak kelihatan sama sekali.
  const renderTransactionCard = (row: BankAgentTransaction) => (
    <div className="space-y-2.5">
      ...
      <div className="flex items-end justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] text-gray-500">Nominal</p>
          <p className="truncate text-base font-bold text-gray-900">{formatCurrency(row.base_amount)}</p>
        </div>
      ...
      {(row.fee > 0 || row.admin_fee > 0) && (
        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs">
          {row.fee > 0 && <span className="text-gray-600">Komisi {formatCurrency(row.fee)}</span>}
          {row.admin_fee > 0 && <span className="text-red-600">Adm bank -{formatCurrency(row.admin_fee)}</span>}
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      {/* Di mobile judul & tombol ditumpuk: dulu dipaksa sebaris sehingga
          judul terhimpit jadi dua baris dan tombol "Buat Transaksi" terpotong
          di tepi kanan layar. */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1">Transaksi setor/tarik tunai & transfer saldo (bank maupun e-wallet)</p>
        </div>
