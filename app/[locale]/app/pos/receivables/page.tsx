...
export default function ReceivablesPage() {
  ...
  // Tampilan kartu untuk layar kecil -- sebagai tabel 9 kolom, kolom
  // Cabang/Total Tagihan/Sudah Dibayar/Status tidak terlihat sama sekali.
  const renderReceivableCard = (row: ReceivableRow) => (
    <div className="space-y-2.5">
      <div className="flex items-start justify-between gap-3">
        {/* min-w-0 wajib: tanpa itu flex child menolak menyusut & nomor
            invoice yang panjang bikin overflow horizontal. */}
        <div className="min-w-0 flex-1">
          <p className="truncate font-mono text-sm font-semibold text-gray-900">{row.sale_number}</p>
      ...
      <p className="truncate text-xs text-gray-500">{row.customer?.name || 'Walk-in'}</p>

      <div className="flex items-end justify-between gap-3 text-xs">
        <div>
          <p className="text-gray-400">Total: {formatCurrency(row.total_amount)}</p>
          <p className="text-green-700">Dibayar: {formatCurrency(row.paid_amount)}</p>
        </div>
        <p className="shrink-0 text-sm font-bold text-red-700">{formatCurrency(row.outstanding_amount)}</p>
      </div>

      <div className="border-t border-gray-100 pt-2.5">
        <Link
          href={`/app/pos/transactions/${row.uuid}`}
          className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-blue-500 px-2 text-xs font-semibold text-white transition-colors hover:bg-blue-600 cursor-pointer"
        >
          Lihat Detail
        </Link>
      </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="min-w-0">
        <h1 className="text-lg sm:text-2xl font-bold text-[#142D52]">Piutang</h1>
        <p className="mt-1 text-sm sm:text-base text-gray-600">Daftar penjualan yang belum lunas dan sisa tagihannya.</p>
      </div>
      ...
      <DataTable
        ...
        renderMobileCard={renderReceivableCard}
      />
    </div>
  );
}
