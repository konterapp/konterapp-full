...
import {
  ...
  createPpobTransactionType,
  updatePpobTransactionType,
} from '@/lib/api/app/ppob-transaction';

interface PpobTransactionTypeFormData {
  name: string;
...
interface PpobTransactionTypeFormProps {
  typeUuid?: string;
  mode: 'create' | 'edit';
}

export default function PpobTransactionTypeForm({ typeUuid, mode }: PpobTransactionTypeFormProps) {
  ...
  const [formData, setFormData] = useState<PpobTransactionTypeFormData>({
    name: '',
    cash_direction: 'out',
    is_active: true,
    sort_order: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result =
        mode === 'edit' && typeUuid
          ? await updatePpobTransactionType(typeUuid, payload)
          : await createPpobTransactionType(payload);

      if (result.status === 'success') {
        const successMsg = mode === 'edit' ? 'Jenis transaksi berhasil diperbarui' : 'Jenis transaksi berhasil ditambahkan';
        toast.success(successMsg);
        router.push('/app/pos/ppob-transaction-types');
      } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">
        {mode === 'edit' ? 'Edit Jenis Transaksi PPOB' : 'Tambah Jenis Transaksi PPOB'}
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input
            ...
            placeholder="Mis. Pulsa, Token Listrik, Paket Data"
          />
        ...
        <div>
          ...
          >
            <option value="out">Kas Masuk (customer bayar ke kasir -- umum: jual pulsa/token/dst)</option>
            <option value="in">Kas Keluar (kasir bayar ke customer -- refund/void)</option>
          </select>
          <p className="mt-1 text-xs text-gray-400">
            Menentukan arah mutasi saldo akun Server PPOB & kas otomatis saat transaksi jenis ini dibuat.
          </p>
        ...
        <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
          <Link href="/app/pos/ppob-transaction-types">
