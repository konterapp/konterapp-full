...
export default function DashboardPage() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    const [salesRes, productsRes, customersRes, purchasesRes, branchesRes, receivablesRes, payablesRes, reportRes, shiftsRes] =
      await Promise.all([
        canViewSales
          ? fetchApi<ListResponse<SaleListItem>>('/api/app/pos/transactions?page=1&per_page=6&sort_by=created_at&sort_order=desc')
          : Promise.resolve(null),
        canViewProducts
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/products?page=1&per_page=1&is_active=true')
          : Promise.resolve(null),
        canViewCustomers
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/customers?page=1&per_page=1')
          : Promise.resolve(null),
        canViewPurchases
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/purchases?page=1&per_page=1&sort_by=created_at&sort_order=desc')
          : Promise.resolve(null),
        canViewBranches
          ? fetchApi<BranchListResponse>('/api/app/pos/branches/list')
          : Promise.resolve(null),
        canViewSales
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/receivables?page=1&per_page=1')
          : Promise.resolve(null),
        canViewPurchases
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/payables?page=1&per_page=1')
          : Promise.resolve(null),
        canViewReport
          ? fetchApi<ReportSummaryResponse>(`/api/app/pos/reports/profit-loss?date_from=${from}&date_to=${to}`)
          : Promise.resolve(null),
        canViewCustomers
          ? fetchApi<ListResponse<unknown>>('/api/app/pos/shifts?page=1&per_page=1')
          : Promise.resolve(null),
      ]);
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl border border-red-100 bg-red-50/60 p-4">
          ...
          <Link href="/app/pos/receivables" className="inline-flex items-center gap-1 mt-3 text-sm text-red-700 font-medium hover:text-red-900">
        ...
        <div className="rounded-xl border border-amber-100 bg-amber-50/70 p-4">
          ...
          <Link href="/app/pos/payables" className="inline-flex items-center gap-1 mt-3 text-sm text-amber-700 font-medium hover:text-amber-900">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-5 gap-4">
        <div className="xl:col-span-3 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/app/pos/transactions" className="text-sm text-[#142D52] font-medium hover:underline">
              Lihat Semua
            </Link>
          </div>
