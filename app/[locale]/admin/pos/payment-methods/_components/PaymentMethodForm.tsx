...
export default function PaymentMethodForm({ paymentMethodId, mode }: PaymentMethodFormProps) {
  ...
  const fetchPaymentMethod = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pos/payment-methods/${paymentMethodId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = mode === 'edit' && paymentMethodId
        ? `/api/admin/pos/payment-methods/${paymentMethodId}`
        : '/api/admin/pos/payment-methods';
      ...
      if (result.status === 'success') {
        ...
        router.push('/admin/pos/payment-methods');
      } else {
        if (result.errors) {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
          <Link href="/admin/pos/payment-methods">
