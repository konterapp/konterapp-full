...
export default function PaymentMethodsPage() {
  ...
  const fetchPaymentMethods = useCallback(async (page: number) => {
    try {
      ...
      const response = await fetch(`/api/admin/pos/payment-methods?${params}`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/pos/payment-methods/${deleteModal.method.uuid}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          {hasPermission('admin.pos.payment-method.update') && (
            <Link
              href={`/admin/pos/payment-methods/${row.uuid}/edit`}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {hasPermission('admin.pos.payment-method.create') && (
          <Link
            href="/admin/pos/payment-methods/create"
