...
export default function CustomersPage() {
  ...
  const fetchCustomers = useCallback(async (page: number) => {
    try {
      ...
      const response = await fetch(`/api/admin/pos/customers?${params.toString()}`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/pos/customers/${deleteModal.customer.uuid}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          {hasPermission('admin.pos.sale.create') && (
            <Link
              href={`/admin/pos/customers/${row.uuid}/edit`}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {hasPermission('admin.pos.sale.create') && (
          <Link
            href="/admin/pos/customers/create"
