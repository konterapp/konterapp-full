...
export default function PurchaseForm({ mode, purchaseUuid }: { mode: PurchaseFormMode; purchaseUuid?: string }) {
  ...
  useEffect(() => {
    ...
    const fetchPurchase = async () => {
      try {
        ...
        const response = await fetch(`/api/admin/pos/purchases/${purchaseUuid}`);
        const result = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let url = '/api/admin/pos/purchases';
    ...
    if (mode === 'create') {
      ...
      method = 'PUT';
      url = `/api/admin/pos/purchases/${purchaseUuid}`;
    ...
    try {
      ...
      if (result.status === 'success' && result.data) {
        ...
        router.push(`/admin/pos/purchases/${targetUuid}`);
        return;
      }
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-6">
        ...
        <div className="flex items-center justify-between border-t border-gray-200 pt-4">
          <Link
            href={mode === 'edit' ? `/admin/pos/purchases/${purchaseUuid}` : '/admin/pos/purchases'}
