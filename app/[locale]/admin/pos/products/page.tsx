...
export default function ProductsPage() {
  ...
  const fetchProducts = async (page: number) => {
    try {
      ...
      const response = await fetch(`/api/admin/pos/products?${params.toString()}`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/pos/products/${deleteModal.product.uuid}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      ...
      render: (_, row) => (
        <div className="flex items-center space-x-2">
          {hasPermission('admin.pos.product.update') && (
            <Link
              href={`/admin/pos/products/${row.uuid}/edit`}
