...
export default function SuppliersPage() {
  ...
  const fetchSuppliers = async (page: number) => {
    try {
      ...
      const response = await fetch(`/api/admin/pos/suppliers?${params}`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/pos/suppliers/${deleteModal.supplier.uuid}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          {hasPermission('admin.pos.supplier.update') && (
            <Link
              href={`/admin/pos/suppliers/${row.uuid}/edit`}
