...
export default function SupplierForm({ supplierId, mode }: SupplierFormProps) {
  ...
  const fetchSupplier = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pos/suppliers/${supplierId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = mode === 'edit' && supplierId ? `/api/admin/pos/suppliers/${supplierId}` : '/api/admin/pos/suppliers';
      ...
      if (result.status === 'success') {
        ...
        router.push('/admin/pos/suppliers');
      } else {
        if (result.errors) {
