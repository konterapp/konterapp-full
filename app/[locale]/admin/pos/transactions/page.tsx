...
export default function TransactionsPage() {
  ...
  const loadFilterOptions = async () => {
    try {
      const [branchRes, pmRes] = await Promise.all([
        fetch('/api/admin/pos/branches/list').then((res) => res.json()),
        fetch('/api/admin/pos/payment-methods?per_page=1000').then((res) => res.json()),
      ]);
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pos/transactions?${params.toString()}`);
  ...
  const columns: Column<Sale>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <Link
          href={`/admin/pos/transactions/${row.uuid}`}
