...
export default function PpobTransactionsPage() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pos/ppob/transactions?${params.toString()}`);
      const result = await response.json();
  ...
  const handleCheckStatus = async (uuid: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/pos/ppob/transactions/${uuid}/check-status`, {
        method: 'POST',
      });
      const result = await response.json();
